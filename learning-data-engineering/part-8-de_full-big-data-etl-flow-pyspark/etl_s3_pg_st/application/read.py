...
class MoveData:
    ...
    def tosql(self):
        ...
        df_st_spark = spark.createDataFrame(df_st)
        print(df_st_spark)
        
        df_st_spark = df_st_spark.\
            withColumn('id', df_st_spark.id.cast(IntegerType())).\
            withColumn('status', df_st_spark.status.cast(IntegerType())).\
            withColumn('date', df_st_spark.date.cast(DateType()))
            #withColumn('date', df_tt_spark.date.cast(DateType())).\

        #Upload the data to pgsql
        try:
            
            df_st_spark.write.format('jdbc').options(
                url='jdbc:postgresql://database.cezaebyc9hil.us-west-1.rds.amazonaws.com:{PORT}/{DB}'.format(PORT = PORT, 
                DB = DB),
                driver='org.postgresql.Driver',
                dbtable='server_title',
