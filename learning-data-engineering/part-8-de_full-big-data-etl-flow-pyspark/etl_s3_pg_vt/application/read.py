...
class MoveData:
    ...
    def tosql(self):
        ...
        validasi_title = endpoint.get_object(
                Bucket = 'g2academy-bucket-yafie-1',
                Key = 'g2_exam3/validasi_title.csv'
        )

        #Create dataframe using pandas and change it into pyspark dataframe
        df_vt = pd.read_csv(validasi_title['Body'])
        df_vt = df_vt.dropna()
        print(df_vt)
        ...
        df_vt_spark = spark.createDataFrame(df_vt)
        print(df_vt_spark)
        
        df_vt_spark = df_vt_spark.\
            withColumn('id', df_vt_spark.id.cast(IntegerType()))
            
        #Upload the data to pgsql
        try:
            
            df_vt_spark.write.format('jdbc').options(
                url='jdbc:postgresql://database.cezaebyc9hil.us-west-1.rds.amazonaws.com:{PORT}/{DB}'.format(PORT = PORT, 
                DB = DB),
                driver='org.postgresql.Driver',
                dbtable='validasi_title',
